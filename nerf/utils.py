...
class Trainer(object):
        ...
        for epoch in range(self.epoch + 1, max_epochs + 1):
            ...
            if self.epoch % self.eval_interval == 0:
                for idx, val in enumerate(self.text_z):
                    ...
                    #if X.rvs(1)[0] ==1:
                    #self.test(test_loader, scene_id = idx)  # Uncomment for 360 gif             
            #GPUtil.showUtilization()
    ...
    def test(self, loader, save_path=None, name=None, write_video=True, scene_id = None):
        ...
        if write_video:
            ...
            imageio.mimwrite(os.path.join(save_path, f'{name}_{scene_id}_rgb.mp4'), all_preds,  quality=8, macro_block_size=1)
            imageio.mimwrite(os.path.join(save_path, f'{name}_{scene_id}_depth.mp4'), all_preds_depth, quality=8, macro_block_size=1)

            from pdb import set_trace
            imageio.mimwrite(save_path +'/'+str(name)+'_{}_rgb.gif'.format(scene_id), all_preds)
            if self.wandb_obj is not None:
