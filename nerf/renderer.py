...
class NeRFRenderer(nn.Module):
    def __init__(self, opt, teacher_flag, teacher_id ):
        super().__init__()
        ...
        if self.cuda_ray:
            ...
            if True:
                ...
                self.density_grid =  [torch.zeros([self.cascade, self.grid_size ** 3]).cuda()  for i in range(num_scenes)]# [CAS, H * H * H]
                self.density_bitfield =  [torch.zeros(self.cascade * self.grid_size ** 3 // 8, dtype=torch.uint8).cuda() for i in range(num_scenes) ]# [CAS * H * H * H // 8]
    ...
    def run(self, rays_o, rays_d, num_steps=128, upsample_steps=128, light_d=None, ambient_ratio=1.0, shading='albedo', bg_color=None, perturb=False, **kwargs):
        ...
        # TODO: Apply the below transformation to all sampled rays_o and rays_d
        yangle = 3 * np.pi / 2
        xangle = 3 * np.pi / 2
        yrot = torch.tensor([
        ...
        yangle2 = np.pi
        yrot2 = torch.tensor([
            [np.cos(yangle2), 0, np.sin(yangle2)],
            [0, 1, 0],
            [-np.sin(yangle2), 0, np.cos(yangle2)],
        ]).to('cuda').reshape(1, 1, 3, 3).float()
    ...
    def update_extra_state(self, scene_id, decay=0.95, S=128 ):
        ...
        if total_step > 0:
            self.mean_count[scene_id] = int(self.step_counter[scene_id,:int(total_step), 0].sum().item() / total_step)
